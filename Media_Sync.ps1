    ...
    foreach ($File in $FolderCOM.items()) {
        ...
        if ($FolderCOM.getDetailsOf($File, 11) -eq "Picture") {
            ...
            if ($FolderCOM.getDetailsOf($File, 12)) {
                ...
                $DateTime = [DateTime]::ParseExact($temp_date, "yyyy-M-d h:mm tt", [System.Globalization.DateTimeFormatInfo]::InvariantInfo, [System.Globalization.DateTimeStyles]::None) ### Converts to proper datetime
                #$DateTime = [DateTime]::ParseExact($temp_date, "yyyy-M-d h:mm tt", $Null) ### Converts to proper datetime
